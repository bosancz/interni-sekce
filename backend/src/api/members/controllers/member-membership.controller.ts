...
export class MemberMembershipController {
	...
	async updateMemberMembership(
		...
		// Only the given year changes: "zaplaceno" records a payment for it (and writes the note and
		// the amount it carries, if the body sent them), "nezaplaceno" removes the one that is there —
		// together with them, as they belong to the payment. The other seasons are left untouched.
		if (body.paid) await this.membershipPayments.setPaid(member, body.year, body);
