...
import { getCzechIban } from "src/helpers/iban";
import { renderQrCodeSvg } from "src/helpers/qr-code-svg";
import { getSpayd } from "src/helpers/spayd";
...
export interface MemberPaymentRequest extends QrCodeLinkPayment {
	...
	message: string;
	/**
	 * Public URL of the QR platba image — our own signed link, answered with the code itself.
	 * No session needed, so it can be mailed; see {@link MemberPaymentRequestService.getQrCodeLinkUrl}.
	 */
	qrCodeUrl: string;
}

const QR_LOGO_PATH = path.resolve("assets/img/san-emblem.svg");
const QR_CODE_SIZE = 512;
const QR_CODE_COLOR = "#2a3478";
...
export class MemberPaymentRequestService {
	 ...
	 *
	 * Diacritics are stripped because SPAYD is an ASCII format, and plain ASCII is also what the
	 * bank shows back on the statement unmangled.
	 */
	 ...
	 * `/api/qr-platba/<vs>/<account>/<bank>/<amount>/<currency>/<signature>`. The payment it asks
	 * for is the one written into the link, so the QR keeps matching the e-mail it went out in;
	 * only the `MSG:` follows the member, whose name may since have been corrected.
	 *
	 * Nothing here may move into a query string. Android's mailto parser (`androidx.core.net.MailTo`)
	 * percent-decodes the whole `mailto:` URI *before* splitting it on `&` and `=` — so the mail
	 * apps built on it cut the body off at the first `&` of such a URL and keep only what stands
	 * between its first two `=`. A member was handed a message ending mid-link at
	 * `…/image?currency`, with no QR, no thanks and no signature. This URL carries everything in
	 * path segments, leaving that parser nothing to split the body on.
	 */
	...
	getQrCodeSvg(member: Member, payment: QrCodeLinkPayment): string {
		const spayd = getSpayd({
			iban: getCzechIban(payment.accountNumber, payment.bankCode),
		...
		return renderQrCodeSvg(spayd, {
			size: QR_CODE_SIZE,
			color: QR_CODE_COLOR,
			background: QR_CODE_BACKGROUND,
			logo: this.getQrCodeLogo(),
		});
	}

	async getQrCodePng(svg: string): Promise<Buffer> {
	...
	private getQrCodeLogo(): string {
		return (this.logo ??= readFileSync(QR_LOGO_PATH, "utf8"));
	}
}
