  ...
  async exportXlsx(members?: Member[]) {
    const schema: Schema<Member> = [
      ...
      },
      {
        ...
        width: 15,
        align: "right",
        value: (member) => (member.birthday ? DateTime.fromISO(member.birthday).toFormat("d. M. yyyy") : undefined),
      },
      {
        column: "Ulice a č. domu",
        type: String,
        width: 20,
        value: (member) => [member.addressStreet, member.addressStreetNo].filter((element) => element).join(" "),
      },
      {
        column: "Obec",
        type: String,
