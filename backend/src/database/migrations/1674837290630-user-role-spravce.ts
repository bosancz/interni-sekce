...
export class userRoleSpravce1674837290630 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TYPE "public"."users_roles_enum" AS ENUM('vedouci', 'clen', 'admin', 'revizor', 'spravce')`,
		);
		await queryRunner.query(
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`CREATE TYPE "public"."users_roles_enum_old" AS ENUM('vedouci', 'clen', 'admin', 'revizor')`,
		);
		await queryRunner.query(
