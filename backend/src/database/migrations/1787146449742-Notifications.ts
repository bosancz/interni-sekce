...
export class Notifications1787146449742 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TYPE "public"."notification_settings_type_enum" AS ENUM('myEvents', 'submittedEvents', 'newEvents', 'newUsers')`,
		);
		await queryRunner.query(
			`CREATE TYPE "public"."notification_settings_channels_enum" AS ENUM('push', 'email', 'inApp')`,
		);
		await queryRunner.query(
			`CREATE TABLE "notification_settings" ("user_id" integer NOT NULL, "type" "public"."notification_settings_type_enum" NOT NULL, "channels" "public"."notification_settings_channels_enum" array, CONSTRAINT "PK_ff98348bd5a8e07a7f3243a969c" PRIMARY KEY ("user_id", "type"))`,
		);
		await queryRunner.query(
			`ALTER TABLE "notification_subscriptions" ADD CONSTRAINT "FK_565bddc96d2492c5bf99274575e" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
		);
		await queryRunner.query(
			`ALTER TABLE "notification_settings" ADD CONSTRAINT "FK_91a7ffebe8b406c4470845d4781" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
		);
	}
