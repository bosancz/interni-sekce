...
export class fixTypes1671535049154 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TYPE "public"."members_achievements_type_enum" AS ENUM('star', 'certificate', 'other')`,
		);
		await queryRunner.query(
			`ALTER TABLE "members_achievements" ADD "type" "public"."members_achievements_type_enum" NOT NULL`,
		);
		await queryRunner.query(`ALTER TABLE "members_contacts" DROP COLUMN "type"`);
		await queryRunner.query(
			`CREATE TYPE "public"."members_contacts_type_enum" AS ENUM('mobile', 'email', 'other')`,
		);
		await queryRunner.query(
			`ALTER TABLE "members_contacts" ADD "type" "public"."members_contacts_type_enum" NOT NULL`,
		);
		...
		await queryRunner.query(
			`ALTER TABLE "events_attendees" ADD "type" "public"."events_attendees_type_enum" NOT NULL`,
		);
		await queryRunner.query(`ALTER TABLE "events" DROP COLUMN "status"`);
		await queryRunner.query(
			`CREATE TYPE "public"."events_status_enum" AS ENUM('draft', 'pending', 'public', 'cancelled', 'rejected')`,
		);
		await queryRunner.query(
