...
- **The DB enum has only `draft`, `pending`, `public`, `cancelled`** (`EventStates` in `event.entity.ts`); `rejected` was dropped from it in migration `1682264978418`. "Vrátit k úpravám" (`POST /events/:id/reject`) therefore sets `draft` + a `statusNote` — nothing records that an event _was_ returned, so `rejected` is unreachable at runtime. `EventStatuses` in `frontend/src/app/core/config/event-statuses.ts` still carries `rejected` and `finalized`, so frontend code that maps over statuses must keep tolerating them.
- `bo-event-progress` (event view, between header and content) renders the workflow as three points — příprava / schvalování / program — filled up to the active one in that status's `EventStatuses.color`. A `draft` event with no leaders shows "Bez vedoucího" in red, mirroring the `bo-pill-danger-dashed` pill on `bo-event-card`. Step 2 shows "Vrácená k úpravám" only if `rejected` ever comes back (see above).
