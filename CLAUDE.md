...
- `bo-event-progress` (event view, between header and content) always shows five points: příprava / schvalování / program, "Ohláška odeslána" and "Uzavření n/3" — filled up to the active one in that status's `EventStatuses.color`, the last two grey until done. A `draft` event with no leaders shows "Bez vedoucího" in red (mirroring `bo-pill-danger-dashed` on `bo-event-card`); `rejected` shows "Vrácená k úpravám" on point 2, which reads "Schválena" once past it (`PAST_STEP_LABELS`). A `statusNote` adds a chat icon opening an `ion-popover` — popover content renders outside view encapsulation, so its styles are `::ng-deep`, as in `home-card-top-leaders`.
- The whole step block is the click target (tinted hover on the label text) and opens a centred dialog via `ModalService.componentModal` like every other modal: `EventAnnouncementModalComponent` and `EventClosureModalComponent`, checkbox forms extending `InputModalComponent`. Closure lists accounting (the one tickable checkbox), report (derived from `event.report`) and gallery (from `event.album`: missing / `draft` / `public` — `getEvent()` joins it), the derived two disabled. The highlight moves to point 4 once the announcement is marked and to point 5 at 3/3; the component calls the API on the modal result and emits `(change)` so `event-view` reloads.
