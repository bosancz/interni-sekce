...
- **A missing or stale `dev.log` means no server is running: you are in a fresh cloud/sandbox session and are expected to start one, so you can actually verify your change.** The rule above only prevents fragmenting an *already running* server; it is not licence to ship unverified.
  - **Node 24 is required.** Angular's engines are `^22.22.3 || ^24.15.0 || >=26.0.0` and the sandbox's default Node 22.22.2 sits one patch below that floor, so every `ng` command dies on the engine check before doing anything. Node 24 lives in `/opt/node24`, installed by the cloud environment's setup script; if `node -v` reports 22 that script did not run, and nothing Angular-related works until it does. Reaching for `nvm` is a dead end — it has no versions installed, and its `PATH` changes do not survive between commands. Because `/opt/node22/bin` sits ahead of `/usr/local/bin` on `PATH`, the setup script repoints the symlinks there too, so `which node` naming a `node22` path while `node -v` says 24 is expected.
