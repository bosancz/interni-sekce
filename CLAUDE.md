...
- **Entries end with the avatars of their authors**, linked to their GitHub profiles (a collapsed duplicate entry carries the authors of all its commits). When the committer is somebody else — usually Claude — their avatar sits behind the author's, smaller (`--avatar-size: 1rem`) and 60 % covered like a collapsed GitHub avatar stack: not a link, and it does not open on hover. One tooltip covers the whole credit — the author, or "Autor a Committer" (e.g. "Kopec a Claude") — so the avatars themselves carry none. Commits committed by GitHub itself (`noreply@github.com`, i.e. its web UI) show the author alone. The layout hangs off `changelog-credit`/`-author`/`-committer`/`-avatar` in `changelog-modal.component.scss`.
