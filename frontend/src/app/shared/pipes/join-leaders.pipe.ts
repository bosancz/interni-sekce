	...
	transform(value: any[], showPhone: boolean = true, html: boolean = true): string {
		...
		const leadersStrings = value
			.map((leader) => {
				...
				if (showPhone && leader.contacts && leader.contacts.mobile)
					leaderString += " (" + formatPhonePipe.transform(leader.contacts.mobile, "short", true) + ")";
				return leaderString;
			})
			.map((leaderString) => (html ? leaderString.replace(/ /g, "&nbsp;") : leaderString));

		return leadersStrings.length > 1
			? leadersStrings.slice(0, leadersStrings.length - 1).join(", ") +
					(html ? " a&nbsp;" : " a ") +
					leadersStrings[leadersStrings.length - 1]
			: leadersStrings[0];
	}
}
