...
export class PhotoGalleryComponent implements OnInit, AfterViewChecked, OnChanges {
	...
	@Output() click = new EventEmitter<BackendApiTypes.PhotoResponseWithLinks>();
	...
	createRows() {
		...
		while (photos.length) {
			...
			let photo: BackendApiTypes.PhotoResponseWithLinks | undefined;
	...
	onPhotoClick(event: MouseEvent, photo: BackendApiTypes.PhotoResponseWithLinks) {
