...
export class ProgramExportService {
	export(events: BackendApiTypes.EventResponseWithLinks[]) {
		const groupedEvents: BackendApiTypes.EventResponseWithLinks[][] = [];
		from(events)
			.pipe(
						...
						children: [
							...
							new TableCell({
								...
								children: [
									...
									new Paragraph({ text: event.description ?? undefined, style: "normal" }),
									new Paragraph({
										text:
