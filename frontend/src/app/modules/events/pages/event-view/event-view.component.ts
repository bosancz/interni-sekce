...
export type EventStatusActions = ExtractExisting<
	keyof BackendApiTypes.EventResponseWithLinks["_links"],
...
export class EventViewComponent implements ViewWillEnter, ViewWillLeave {
	...
	async updateEvent(data: Partial<BackendApiTypes.EventUpdateBody>) {
	...
	async leadEvent(event: BackendApiTypes.EventResponseWithLinks) {
	...
	private async eventStatusAction(event: BackendApiTypes.EventResponseWithLinks, action: EventStatusActions) {
	...
	private async deleteEvent(event: BackendApiTypes.EventResponseWithLinks) {
	...
	private async restoreEvent(event: BackendApiTypes.EventResponseWithLinks) {
		await this.api.EventsApi.restoreEvent(event.id);
		await this.loadEvent(event.id);
		this.toastService.toast("Akce obnovena");
	}

	private setActions(event: BackendApiTypes.EventResponseWithLinks) {
		this.actions = [
			{
				text: "Vést akci",
				color: "success",
