...
export class EventAttendeesComponent implements OnInit, OnDestroy, OnChanges {
	...
	attendees: BackendApiTypes.EventAttendeeResponseWithLinks[] = [];
	leaders: BackendApiTypes.EventAttendeeResponseWithLinks[] = [];
	...
	private async loadAttendees(event?: BackendApiTypes.EventResponseWithLinks | null) {
		if (!event) {
			this.attendees = [];
			this.leaders = [];
			return;
	...
	private sortAttendees(members: BackendApiTypes.EventAttendeeResponseWithLinks[]) {
		members.sort((a, b) => {
	...
	async addAttendee() {
		...
		if (member) {
			...
			this.attendees.push({
				...
				memberId: member.id,
				member,
			} as BackendApiTypes.EventAttendeeResponseWithLinks);
	...
	async removeAttendee(attendee: BackendApiTypes.EventAttendeeResponseWithLinks) {
		if (!this.event) return;
