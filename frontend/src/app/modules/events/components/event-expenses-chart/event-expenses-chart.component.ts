...
export class EventExpensesChartComponent implements OnInit, OnChanges {
	...
	@Input() expenses?: BackendApiTypes.EventExpenseResponseWithLinks[];
	...
	totalByType: Record<BackendApiTypes.EventExpenseTypesEnum, number> = {
		accommodation: 0,
		food: 0,
		material: 0,
		other: 0,
	...
	private async updateChart() {
		...
		const data: ChartData<"doughnut">["datasets"][0]["data"] = Object.keys(EventExpenseTypes).map((type) => {
			return this.getTotalExpenseByType(type as BackendApiTypes.EventExpenseTypesEnum) / this.persons / this.days;
		});
	...
	private getTotalExpenseByType(type: BackendApiTypes.EventExpenseTypesEnum) {
		return (
