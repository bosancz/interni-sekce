...
export type MemberSelectorType = BackendApiTypes.MemberResponse | BackendApiTypes.MemberResponse[] | null;
...
export class MemberSelectorComponent implements OnInit, ControlValueAccessor, AfterViewInit, OnDestroy {
	value: BackendApiTypes.MemberResponse[] = [];
	...
	addMember(member: BackendApiTypes.MemberResponse) {
		const i = this.value.findIndex((item) => item.id === member.id);
		if (i !== -1) return;
		return this.updateValue([...this.value, member]);
	}

	removeMember(member: BackendApiTypes.MemberResponse) {
		const i = this.value.findIndex((item) => item.id === member.id);
		if (i === -1) return;
