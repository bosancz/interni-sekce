...
export class UsersEditComponent implements OnInit {
	...
	async loadUser(userId: number) {
		this.user = await this.api.get("/api/users/{id}", { params: { id: userId } }).then((res) => res.data);
	}

	async loadMembers() {
		let members = await this.api.get("/api/members", { query: {} }).then((res) => res.data);
	...
	async saveUser() {
		...
		await this.api.patch("/api/users/{id}", userData, { params: { id: this.user.id } });
